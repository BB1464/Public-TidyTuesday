...
```{r, Import_the_image}
path_img=setwd(dir = '2022/2022-04-26-Week-17')

img_d <- tibble(
  media = c("Linkedin", "Twitter"),
  img = str_c(path_img, c("linkedIn.JPEG", "twitter.JPEG"))
)
```
...
p1 + p2 + p3 + plot_layout(design = design) + plot_annotation(title = "Notebook Titles")
```
...
for (i in seq(nrow(p_map))){
  ...
  p <- p +
    annotation_custom(
      rasterGrob(image_read(p_map$img[i]), 
                 width = unit(1, "npc"),  # 
                 height = unit(1, "npc")),
      xmin = p_map$xmin[i], 
      xmax = p_map$xmax[i],
